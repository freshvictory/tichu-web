...
changeTurnScore scorer score =
  if score >= -25 && score <= 125 then { scorer | vertTurnScore = score } else scorer


changePlayerBet : Scorer -> Player -> Bet -> Scorer
changePlayerBet scorer player bet =
  case player of
    North -> { scorer | north = (player, bet) }
    South -> { scorer | south = (player, bet) }
    East -> { scorer | east = (player, bet) }
    West -> { scorer | west = (player, bet) }
...
undo scorer =
  let
    ((vS, hS), history) = case scorer.history of
      [] ->
        ((0, 0), [(0, 0)])
      [x] ->
        (x, [(0, 0)])
